...
#| 
num_bins <- 10
mat_hist <- ggplot(anpp, aes(x = MAT))+
  geom_histogram(color = 'black',
                 fill = 'lightgrey',
                 bins = num_bins)+
  geom_vline(aes(xintercept = mean(MAT)),
             linetype = 'dashed',
             color = 'red')+
  labs(y = 'Count')
map_hist <- ggplot(anpp, aes(x = MAP))+
  geom_histogram(color = 'black',
                 fill = 'lightgrey',
                 bins = num_bins)+
  geom_vline(aes(xintercept = mean(MAP)),
             ...
             color = 'red')+
  labs(y = 'Count')
anpp_hist <- ggplot(anpp, aes(x = ANPP))+
  geom_histogram(color='black',
                 fill='lightgrey',
                 bins = num_bins)+
  geom_vline(aes(xintercept = mean(ANPP)),
             linetype = 'dashed',
             color = 'red')+
  labs(y = 'Count')
f1 <- anpp_hist+map_hist+mat_hist+plot_layout(axis_titles = 'collect') # edit this later for fig panel labels
f1

f1&facet_wrap(facets = vars(region))
```
...
mat_anpp <- ggplot(anpp, aes(x = MAT, y = ANPP, col = region))+
 ...
 # geom_smooth(se = FALSE) # adds LOESS scatterplot smoother
